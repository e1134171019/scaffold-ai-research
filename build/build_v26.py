...
SCHEME3_UPDATE_JS = ROOT / "scheme3-update-v44.js"
SCHEME3_UPDATE_CSS = ROOT / "scheme3-update-v44.css"
...
def inject_scheme1_update(old: bytes) -> bytes:
    """Replace Scheme 1 wording and technical detail after ACT 04 is assembled."""
    html = old.decode("utf-8")
    if "scheme1-update-v42" in html:
        return old
    if html.count("</head>") != 1 or html.count("</body>") != 1:
        raise RuntimeError("Generated site head/body boundary is missing or ambiguous")

    css = SCHEME1_UPDATE_CSS.read_text(encoding="utf-8").strip()
    js = SCHEME1_UPDATE_JS.read_text(encoding="utf-8").strip()
    html = html.replace(
        "</head>",
        f"<style>\n{css}\n</style>\n</head>",
        1,
...
def inject_scheme3_update(old: bytes) -> bytes:
    """Separate the Scheme 3 reader story from its engineering decision pipeline."""
    html = old.decode("utf-8")
    if "scheme3-update-v44" in html:
        return old
    if html.count("</head>") != 1 or html.count("</body>") != 1:
        raise RuntimeError("Generated site head/body boundary is missing or ambiguous")

    css = SCHEME3_UPDATE_CSS.read_text(encoding="utf-8").strip()
    js = SCHEME3_UPDATE_JS.read_text(encoding="utf-8").strip()
    html = html.replace(
        "</head>",
        f"<style>\n{css}\n</style>\n</head>",
        1,
...
def main() -> None:
    ...
    scheme3 = inject_scheme3_update(scheme3)
